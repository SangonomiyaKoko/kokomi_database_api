...
ship_name_data = json.load(open(os.path.join(
    file_path, 'data', 'json', 'ship_name.json'), "r", encoding="utf-8"))
logging.basicConfig(
    level=logging.INFO,
        ...
        if 'hidden_profile' in temp_data['data'][aid]:
            ...
            for ship_id, ship_data in temp_data['data'][aid]['statistics'].items():
                ...
                if ship_id not in ship_name_data:
                    continue
                if select[0] == [] or ship_name_data[ship_id]['tier'] in select[0]:
                    if select[1] == [] or ship_name_data[ship_id]['type'] in select[1]:
                        if select[2] == [] or ship_name_data[ship_id]['nation'] in select[2]:
